...
print("File length: " + str(len(data2)))
print("Getting rid of uneccessary tags")
...
for t in taglist:
    ...
    name = t[2]
    offset = len(stop)
    print("----------------- " + name + " tag replacement started -------------------")
    print("File length: " + str(len(data2)))
    while r > 0:
        r = data2.find(start)
        if(r > i):
    ...
    print("----------------- " + name + " tag replacement done -------------------\n")

arr = data2.split("\n")
arr1 = []
for i in arr:
    arr1.append(i.split("/*^v^*/"))
print(str(len(arr)) + " total lines.")

with open("test.csv","w+", encoding='utf-8') as my_csv:
